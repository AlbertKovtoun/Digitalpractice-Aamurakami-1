...
float movingNoise(vec2 p) {
  ...
  float x = fractalNoise(p + uTime * 0.1);
  float y = fractalNoise(p - uTime * 0.1);
  ...
  return movingNoise(p + vec2(x, y));
    ...
    shader.fragmentShader = shader.fragmentShader.replace(
      ...
      /* glsl */ `
          ...
          diffuseColor.rgb = vec3(vertexHeight) * ((1.0 - min(1.0, mirror)) + newMerge.rgb * mixStrength);
          `
    )
  }
}
